...
class User
{
	...
	function RegistrarM($nombre,$apellidoP,$apellidoM,$matricula,$pass,$correo)
	{
		...
		$rol="Maestro";
		$pass2=md5($pass);
		$query="INSERT INTO usuario (`Matricula`, `Nombre`, `Pass`, `ApellidoP`, `ApellidoM`, `Rol`, `Correo`, `Activo`)  values ( '$matricula', '$nombre', '$pass2', '$apellidoP', '$apellidoM', '$rol', '$correo', '1' )";
        mysqli_query($Conexion,$query);
    ...
    function RegistrarAd($nombre,$apellidoP,$apellidoM,$matricula,$pass,$correo)
	{
		require ('Conexion.php');
		$rol="Administrador";
        $activo='1';
	...
	function Baja($matricula){
        require ('Conexion.php');
        $query="UPDATE usuario SET Activo='0' WHERE Matricula='".$matricula."'";
        mysqli_query($Conexion,$query);
	}
